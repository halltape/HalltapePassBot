...
2. Укажите в `.env` токен вашего Telegram-бота:
```env
TELEGRAM_TOKEN=123456:ABCDEF...
```

3. Соберите Docker-образ из корня проекта:
```bash
docker build -t halltape-pass-bot .
```

4. Запустите контейнер, передав переменные окружения из `.env`:
```bash
docker run --rm --env-file .env halltape-pass-bot
```

> Бот запускается командой `python main.py` внутри контейнера и не требует проброса портов.
***
**Описание**
